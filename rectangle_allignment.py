...
for rectContour in rectangleContours:
    cv2.rectangle(rotatedImgContour, (rectContour['BBpoints'][0], rectContour['BBpoints'][1]), (rectContour['BBpoints'][2], rectContour['BBpoints'][3]), (219, 67, 119), 5)
...
for i in range(0, len(lineContours)):
    ...
    cv2.rectangle(rotatedImgContour, (int(x_start), int(y_start)), (int(x_end), int(y_end)), (219, 67, 119), 3)

    cv2.putText(rotatedImgContour, "Rectangle Alligned Image", (120, 28), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
    cv2.imshow("Rectangular Allignment Window", rotatedImgContour)
